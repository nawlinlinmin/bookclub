...
RSpec.describe Book, type: :model do
  ...
  context 'When a book is deleted' do
    it "deletes reviews associated with that book" do
    ...
    it "deletes comments associated with that book" do
      comment = Comment.create(body: "body", book_id: @book.id, user_id: @user.id)
      expect{ @book.destroy }.to change{ Comment.count }.by(-1)
    end

    it "deletes favorites associated with that book" do
